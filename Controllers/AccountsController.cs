...
namespace Controllers
{
    public class AccountsController : Controller
    {
        ...
        public ActionResult DeleteProfil()
        {
            ...
            if (connectedUser != null)
            {
                // remove likes from all medias for this user and broadcast updates / deletions
                try
                {
                    var allMedias = DB.Medias.ToList();
                    var hubContext = Microsoft.AspNet.SignalR.GlobalHost.ConnectionManager.GetHubContext<Hubs.LikesHub>();
                    // remove likes
                    foreach (var m in allMedias)
                    {
                        if (m.Likes != null && m.Likes.Contains(connectedUser.Id))
                        {
                            m.Likes.RemoveAll(id => id == connectedUser.Id);
                            DB.Medias.Update(m);
                            try { hubContext.Clients.All.mediaLiked(m.Id, m.LikesCount, m.Likes != null ? m.Likes.ToArray() : new int[0], m.LikedByUsers.Select(u=>u.Name).ToArray()); } catch { }
                        }
                    }
                    // delete owned medias
                    var owned = allMedias.Where(x => x.OwnerId == connectedUser.Id).ToList();
                    foreach (var om in owned)
                    {
                        DB.Medias.Delete(om.Id);
                        try { hubContext.Clients.All.mediaDeleted(om.Id); } catch { }
                    }
                }
                catch
                {
                    // fallback: best-effort cleanup without broadcasting
                    try
                    {
                        var allMedias = DB.Medias.ToList();
                        foreach (var m in allMedias)
                        ...
                        var owned = allMedias.Where(x => x.OwnerId == connectedUser.Id).ToList();
                        foreach (var om in owned) DB.Medias.Delete(om.Id);
                    }
                    catch { }
                }
        ...
        public ActionResult DeleteUser(int id)
        {
            ...
            if (id != 1)
            {
                ...
                if (user != null)
                {
                    ...
                    // First remove user's likes from all medias
                    try
                    {
                        var allMedias = DB.Medias.ToList();
                        foreach (var m in allMedias)
                        {
                            if (m.Likes != null && m.Likes.Contains(user.Id))
                            {
                                m.Likes.RemoveAll(uid => uid == user.Id);
                                DB.Medias.Update(m);
                            }
                        }
                        // delete all medias owned by user
                        var owned = allMedias.Where(x => x.OwnerId == user.Id).ToList();
                        // broadcast deletions and delete
                        try
                        {
                            var hubContext = Microsoft.AspNet.SignalR.GlobalHost.ConnectionManager.GetHubContext<Hubs.LikesHub>();
                            foreach (var om in owned)
                        ...
                        {
                            // fallback: delete without broadcasting
                            foreach (var om in owned) DB.Medias.Delete(om.Id);
                        }
                    }
                    catch { }
