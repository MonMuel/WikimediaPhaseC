...
public class MediasController : Controller
{
    ...
    public ActionResult ToggleMediaLike(int id)
    {
        try
        {
            ...
            if (media.Likes == null) media.Likes = new System.Collections.Generic.List<int>();

            bool liked;
            if (media.Likes.Contains(userId))
            {
                media.Likes.Remove(userId);
                liked = false;
                DB.Events.Add("Unlike", media.Title);
            }
            else
            {
                media.Likes.Add(userId);
                liked = true;
                DB.Events.Add("Like", media.Title);
            }

            DB.Medias.Update(media);

            // debug log current likes for troubleshooting
            try
            {
                Debug.WriteLine($"ToggleMediaLike: mediaId={media.Id} likes=[{string.Join(",", media.Likes ?? new System.Collections.Generic.List<int>())}]");
            }
            catch { }
            ...
            try
            {
                var context = Microsoft.AspNet.SignalR.GlobalHost.ConnectionManager.GetHubContext<Hubs.LikesHub>();
                var userIds = media.Likes != null ? media.Likes.ToArray() : new int[0];
                var userNames = media.LikedByUsers.Select(u => u.Name).ToArray();
                context.Clients.All.mediaLiked(media.Id, media.LikesCount, userIds, userNames);
            }
            catch { /* if SignalR not configured, ignore */ }

            var users = media.LikedByUsers.Select(u => u.Name).ToArray();
            return Json(new { success = true, liked = liked, count = media.LikesCount, users = users }, JsonRequestBehavior.AllowGet);
        }
    ...
    private List<Media> _getItems(int index, int nbItems)
    {
        try
        {
            ...
            if (search)
            {
                ...
                string SelectedOwner = (string)Session["SelectedOwner"];
                if (!string.IsNullOrEmpty(SelectedOwner))
                {
                    if (int.TryParse(SelectedOwner, out int ownerId))
                    {
    ...
    public ActionResult GetMediaLikeHeader(bool forceRefresh = false)
    {
        try
        {
            InitSessionVariables();
            ...
            if (DB.Users.HasChanged || DB.Medias.HasChanged || forceRefresh)
            {
                return PartialView("GetMediaLikeHeader", Media);
            }
            return null;
        }
        catch (System.Exception ex)
